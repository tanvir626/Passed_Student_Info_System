    ...
    public partial class WebForm1 : System.Web.UI.Page
    {
        ...
        protected void btnsave_OnClick(object sender, EventArgs e)
        {
            ...
            if (fpslip.HasFile)
            {
               ...
               // fpslip.SaveAs(Server.MapPath("image/") + Path.GetFileName(fpslip.FileName));
            ...
            slippath = "image/" + Path.GetFileName(fpslip.FileName);
            mampath = "image/" + Path.GetFileName(fpmemberimage.FileName);
            con.Open();
            SqlCommand com = new SqlCommand("INSERT INTO convocationform (Registration_Date, Member_Name, Gender, Birthday, Blood_Group, Contact_Number, Email, Education_Type, Present_Profession, Previour_Profession, Rergistration_Fee, Deposit_Slip, member_img,Deposit_ref_no,passing_year) VALUES ('" + DateTime.Today.ToString("yyyy-MM-dd") + "', '"+ txtmembername.Text + "', '"+ ddlgender.SelectedValue + "', '"+ txtdateofbirth.Text + "', '"+ ddlbloodgroup.SelectedValue + "', '"+ txtmobile.Text + "', '"+ txtemail.Text + "','"+ ddlfrom.Text + "', '"+ txtpresentprofession.Text + "', '"+ txtpreviousprofession.Text + "', '" + txtregistrationfee.Text + "', '"+ slippath.ToString() + "', '"+ mampath.ToString() + "','"+txtdepositreference.Text+"',+'"+txtlpass.Text+"')", con);
            com.ExecuteNonQuery();
            con.Close();
            cls();
        ...
        void Preloder()
        {
            ...
            while(dr.Read())
            {
                ...
                Image1.ImageUrl= dr.GetValue(12).ToString();
